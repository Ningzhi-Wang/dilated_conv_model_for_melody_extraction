...
def create_acoustic_model_fn(reg=1e-4):
    ...
    with tf.name_scope('local'):
        for layer_idx in range(4):
            with tf.name_scope('layer_{}'.format(layer_idx)):
                ...
                outputs.reshape([None, None, 540, None])
    ...
    with tf.name_scope('global'):
        ...
        outputs = tf.keras.ops.pad(outputs, [[0, 0], [0, 0], [240, 60], [0, 0]])
        outputs = tf.keras.layers.Conv2D(
            name=get_name_scope() + 'conv',
            kernel_size=[1, 97],
            dilation_rate=[1, 5],
        ...
        outputs.reshape([None, None, 360, 128])
        outputs = bn_relu_fn(outputs)
        outputs = tf.keras.layers.Dropout(
            name=get_name_scope() + 'dropout',
            rate=.2
    ...
    with tf.name_scope('output'):
        ...
        with tf.name_scope('output'):
            ...
            outputs.reshape([1, None, 360, 1])
            outputs = tf.keras.ops.squeeze(outputs, axis=[0, -1])
